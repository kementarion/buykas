...
## 2025-12-14 区块浏览器 kas.fyi 关闭

因为运营费用不菲，以及对社区一件 KRC20 事件的不満（kas.fyi 拒绝添加一个明显炒作诈骗的 meme 币的信息，引起一些人的对 kas.fyi 的攻击，认为一个数据平台不应该做审查）。kas.fyi 的作者 CryptoK 决定将 [https://kas.fyi](https://kas.fyi) 下线，并注销了自己的推特账号 ([https://x.com/cryptok777](https://x.com/cryptok777)) 和 kas.fyi 的官方推特账号。

::: info Yonatan 对此事的回应

Regarding kas.fyi: unfortunately the owner was somewhat burnt by previous community interactions around a certain krc20 issue, and is no longer interested in receiving support, instead insists on selling (price is legit, it's his efforts, but we can fund a dev for a full year with same budget).

Our plan B is to fund Alex (KIP16 author) to help fill the gap kas.fyi created - whether he does so via a new platform or through existing ones, we discussed, but I’ll leave to him to communicate. There is clear value in redundant explorer infra, and afaik both existing ones use the kaspa.org api.

I want to reiterate: if you have created infra for Kaspa that is already used and loved, and serves as a public good, we* will find sources to fund you. Please message me before you are burnt out and want to exit and sell - rather when you are still committed to the project and just need some kas support and a nudge of deserved recognition for your efforts. But again pls refer to the beginning of this paragraph and comprehend the context of this.
...
- 来源：[https://discord.com/channels/599153230659846165/916662870379491328/1449862229158531193](https://discord.com/channels/599153230659846165/916662870379491328/1449862229158531193)

:::

如今，[https://kas.fyi](https://kas.fyi) 域名重定向到了另一个区块链浏览器：[https://kaspa.stream/](https://kaspa.stream/)

kas.fyi 是最早的也是最受欢迎的 Kaspa 区块浏览器，在 Kaspa 社区极具影响力。kas.fyi 的关闭是一件非常遗憾的事件。
...
## 2025-12-28 k-social 上线

[https://k-social.network/](https://k-social.network/) 是一个类似于微博、推特的社交网络。

所有的消息发送、个人信息更新，都是一笔 Kaspa 交易。一切信息上链。真正的 **Web 3.0** 。

<img :src="$withBase('/kas/k-social-indexer.png')" />

::: info 翻译：

无论如何，K 网络具备以下缓解措施（K 网络通过以下方式赋予了用户数据自主权）：

1. 个人索引器：您可以运行自己的个人索引器节点，无需的数据将自动在本地被过滤或删除。

2. 公共索引器：公共索引器的运营者管理着自己的数据库。这意味着，如果他们愿意，也可以清除其数据库中的任何不需要的数据。

您可以查看社区开发的K索引器示例仓库（https://github.com/thesheepcat/K-indexer/tree/master?tab=readme-ov-file），其中已提供了相关的工具。

关键在于：在像 K 这样的去中心化网络中，并非“无人管理”，而是“管理权完全交给了用户”——你对自己的数据和节点拥有完全的自主权。
